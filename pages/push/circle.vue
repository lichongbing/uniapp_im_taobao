    ...
    <view class="moments__post"
      ...
      <view class="post_right">
        ...
        <view class="toolbar">
          ...
          <view class="timestamp" @tap="deletes(index)" style="margin-left: 30upx;color: #FF7600;" v-if="post.uid==my_data.id">删除</view>
        ...
        <!-- 赞／评论区 -->
        <view class="post-footer" v-if="post.comments.length!=0||post.like.length!=0">
...
export default {
  ...
  methods: {
    ...
    goCircleChat () {
	  ...
	  console.log(_data.data('no_reader_circle_chat_num', 0))
	...
	deletes (index) {
		let _this = this;
		var circle_data = _data.localData('circle_data');
		            ...
		            _this.$httpSend({
		              ...
		              success (res) {
						  for (var i=0; i<circle_data.length;i++) {
							  console.log("123")
						  	if (circle_data[i].post_id == _this.posts[index].post_id) {
						  		circle_data.splice(i,1);
						  		uni.setStorageSync('circle_data', circle_data);
								_this.posts = circle_data;
								uni.showToast({
								    title: '已删除',
								    duration: 2000
