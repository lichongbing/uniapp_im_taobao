<template>
  ...
  <view class="page"
          ...
          <view class="row"
            ...
            <block v-if="row.type==0">
              ...
              <view class="my"
                ...
                <!-- 左-消息 -->
                <view class="left"
                      @longtap="msgAction(row.msg.id +'',row.msg.user_info.uid + '',row.msg.type,row.msg.content)"
                      :id="row.msg.id">
                  <!-- 文字消息 -->
                  <view v-if="row.msg.type==0"
                        class="bubble">
						<view v-if="row.msg.content.text" class="" v-html="setEnter(row.msg.content.text)">
	...
	<view id="copyH5" style="display: none;">
...
setEnter(content){
	content=content.replace(/%&回车%&/g,"<br>")
	return content
},
      inputText(e){
        //截取最后一位
      ...
      autoHeight(type,not_return){
		...
		switch(uni.getSystemInfoSync().platform){
		    case 'android':
				...
				break;
		    case 'ios':
				if(not_return == 0&&type == 'hide'){
					this.$nextTick(() => {
						_this.scrollTop = addr+200;
					})
				}else if(not_return == 0&&type == 'show'){
					this.$nextTick(() => {
						_this.scrollTop = (new Date()).getTime()*2;
					})
				}
				break;
		}
		...
		} else if(not_return == 1) {
			this.$nextTick(() => {
				_this.scrollTop = addr+200;
			})
		} */
        /* if(not_return == 0){
			console.log('执行高度调整', addr)
			this.$nextTick(() => {
				_this.scrollTop =(new Date()).getTime();
			})
      ...
      msgAction (id, user_id,type,content) {
        let _this = this;
        ...
        //     break;
        // }
		var itemList=type==0?['撤回','收藏','复制']:['撤回','收藏']
        uni.showActionSheet({
          itemList: itemList,
          success: function (res) {
            switch (res.tapIndex) {
              ...
              case 1:
				...
				case 2://复制
						...
						uni.setClipboardData({
							data: content.text.match(/<div[\s\S]*;">([\s\S]*)<\/div\>/)[1].replace(/<\/div\>/g,"")
						});
						// #endif
						// #ifdef H5
						  var oInput = document.createElement('input');
						  oInput.value = content.text.match(/<div[\s\S]*;">([\s\S]*)<\/div\>/)[1].replace(/<\/div\>/g,"");
	  ...
	  Position(){
		  uni.getLocation({
		      ...
		      success: function (res) {
				  ...
				  let msg = { url: 'https://apis.map.qq.com/ws/staticmap/v2/?center='+res.latitude+','+res.longitude+'&zoom=10&size=600*300&maptype=roadmap&key=R7WBZ-F4DL4-DW3UP-DZIEC-7DZNZ-OOF4A' };
				  this.sendMsg(msg, 2);
		      },fail:function(err){
		  		console.log(err)
				}
          ...
          switch (type) {
            ...
            case 0:
              let msg = this.textMsg.replace(/</g, '&lt;');
			   msg = JSON.parse(JSON.stringify(msg).replace(/\\n/g, '%&回车%&')) 
              sendMsg = { text: msg };
              callback();
              break;
                  /** 语音/图片/视频/文件 */
